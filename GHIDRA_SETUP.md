...
## Ghidra 12.x and Jython

Ghidra 11.3+ made PyGhidra (real CPython 3) the default Python engine. A classic Jython-style script run without saying so gets routed to PyGhidra, which fails with `"Ghidra was not started with PyGhidra. Python is not available"` if no Python interpreter is bridged in.

The tempting fix — adding `# @runtime Jython` as the script's first line — turns out not to be enough on its own: Ghidra 12.x no longer bundles Jython at all by default, so that directive just produces a clearer error instead (`"In order to use Jython based scripts you must install the Jython Ghidra Extension, or (recommended) port your script to PyGhidra or Java"`). Ghidra's own recommendation is the right call here.

**Actual fix used in this repo:** both scripts in `ghidra_scripts/` start with `# @runtime PyGhidra` instead. PyGhidra scripts run through the Script Manager get the same flat-API globals Jython scripts did (`currentProgram`, `monitor`, `createLabel`, `getFunctionAt`, `createFunction`, `setEOLComment`, etc.) automatically, so this was a one-line port for both files — no logic changes needed, since neither script used any Python-2-only syntax. The one thing to watch for in general when porting a Jython script: drop any `.encode("utf-8")` calls on strings passed into Ghidra's Java API — under real Python 3 that produces a `bytes` object where the API wants a `str`, which behaves differently than it did under Jython/Python 2. Neither script here needed that fix, but it's the thing most likely to bite next time.
